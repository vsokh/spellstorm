...
| **Defense Field** | **Discharge** (RMB) — hold to channel a 180-radius static dome around yourself for up to 2.5s. On activation deals **10 dmg + 2s stun** to every enemy inside and detonates static marks (+3 dmg, +0.5s stun per stack). While held, every 0.2s it re-stuns and deals **2 dmg** to any enemy in the field (up to 25 tick dmg over a full channel on a pinned target) — anything that tries to walk in gets locked down the moment it crosses the boundary, and incoming enemy projectiles are destroyed. Movement slowed to 60%. Breaks if you take 5+ damage in a single hit. 22 mana, 5s cd. |
